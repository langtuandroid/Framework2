...
namespace UnityExtensions
{
    ...
    /// Queued task
    /// </summary>
    internal interface IQueuedTask<TTask> where TTask : IQueuedTask<TTask>
    {
        /// <summary>
    ...
    internal abstract class QueuedTask<TTask> : IQueuedTask<TTask> where TTask : QueuedTask<TTask>
    {
    ...
    /// </summary>
    internal class TaskQueue<TTask> where TTask : IQueuedTask<TTask>
    {
        LinkedList<TTask> _tasks = new LinkedList<TTask>(8);
        bool _processing = false;

        internal bool hasTask => _tasks.count > 0;

        internal int taskCount => _tasks.count;

        internal LinkedList<TTask> tasks => _tasks;

        internal event Action<TTask> taskCompleted;

        /// <summary>
        /// Enqueue a new task.
        /// </summary>
        internal bool Enqueue(TTask task)
        {
        ...
        /// </summary>
        internal void RemoveUnprocessed()
        {
            if (_tasks.count > 1)
            {
