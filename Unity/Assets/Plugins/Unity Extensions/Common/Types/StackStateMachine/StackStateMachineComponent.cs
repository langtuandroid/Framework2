...
namespace UnityExtensions
{
    ...
    public class StackStateMachineComponent<T> : BaseStackStateComponent where T : class, IStackState
    {
        ...
        /// </summary>
        internal float currentStateTime => (float)_currentStateTime;


        /// <summary>
        /// Total time since entering the current state.
        /// </summary>
        internal double currentStateTimeDouble => _currentStateTime;


        /// <summary>
        /// Current state (unusable if stateCount is zero)
        /// </summary>
        internal T currentState => _states[_states.Count - 1];


        /// <summary>
        /// The state under current state (unusable if stateCount is zero or one)
        /// </summary>
        internal T underState => _states[_states.Count - 2];


        /// <summary>
        /// Get the state in the stack by specified index.
        /// </summary>
        internal T GetState(int index)
        {
        ...
        /// </summary>
        internal void PushState(T newState)
        {
#if DEBUG
            if (_duringSetting)
            {
        ...
        /// </summary>
        internal void PopStates(int count)
        {
            while (count > 0)
            {
                PopState();
    ...
    internal class StackStateMachineComponent : StackStateMachineComponent<BaseStackStateComponent>
    {
        internal TimeMode timeMode = TimeMode.Unscaled;
