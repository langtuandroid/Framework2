...
public class TowerPlacementGrid : MonoBehaviour, IPlacementArea
{
    ...
    /// <returns><see cref="int2"/> containing the grid coordinates corresponding to this location.</returns>
    public int2 WorldToGrid(Vector3 worldLocation, int2 sizeOffset)
    {
    ...
    private Nullable<int2> tempUseGridPos;
    private int2? tempUseSize;

    public void TempUse(int2 gridPos, int2 size)
    {
        if (tempUseGridPos.HasValue && gridPos.Equals(tempUseGridPos.Value) && size.Equals(tempUseSize.Value)) return;
        ClearTempUse();
        int2 extents = gridPos + size;
        for (int y = gridPos.y; y < extents.y; y++)
        {
        ...
        tempUseGridPos = gridPos;
        tempUseSize = size;
    }

    public void ClearTempUse()
    {
        if (tempUseGridPos != null)
        {
            Clear(tempUseGridPos.Value, tempUseSize.Value);
            tempUseGridPos = null;
            tempUseSize = null;
        }
    }
    ...
    protected void SetUpGrid()
    {
        ...
        if (tileToUse != null)
        {
            ...
            for (int y = 0; y < dimensions.y; y++)
            {
                ...
                {
                    Vector3 targetPos = GridToWorld(new int2(x, y), new int2(1, 1));
                    targetPos.y += 0.01f;
    ...
    void OnValidate()
    {
            ...
            dimensions.y <= 0)
        {
            ...
            dimensions = new int2(Mathf.Max(dimensions.x, 1), Mathf.Max(dimensions.y, 1));
        }
